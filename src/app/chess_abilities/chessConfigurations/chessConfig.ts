...
export const useChessGame = () => {
    ...
    for (const move of moves) {
      newSquares[move.to] = {
        background: chessGame.get(move.to) && chessGame.get(move.to)?.color !== chessGame.get(square)?.color 
          ? 'radial-gradient(circle, rgba(0, 128, 0, 0.5) 85%, transparent 85%)' 
  ...
  const onSquareClick = useCallback(({ square, piece }: SquareHandlerArgs) => {
    ...
    setMoveFrom('');
    setOptionSquares({});
  }, [moveFrom, getMoveOptions, chessGame, makeRandomMove]);

  // Your exact onPieceDrop logic, but now reusable
